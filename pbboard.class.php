...
if (is_array($CALL_SYSTEM))
  {
    ...
    if (CLASS_NAME == "PowerBBBannedMOD" or isset($_GET["page"]) == "register")
      {
    ...
    if (CLASS_NAME == "PowerBBProfileViewerMOD" or isset($_GET["page"]) == "profile")
      {
    ...
    function __construct()
      {
        ...
        $this
            ->DB
            ->SetInformation($config["db"]["server"], $config["db"]["username"], $config["db"]["password"], $config["db"]["name"], $config["db"]["dbtype"], $config["db"]["encoding"], $config["db"]["prefix"]);
        ...
        if (isset($this->_COOKIE[$this->_CONF["lang_cookie"]]))
          {
            $lang_cookie  = $this
                ->sys_functions
                ->CleanSymbols($this->_COOKIE[$this->_CONF["lang_cookie"]]);
            $lang_cookie  = $this
                ->sys_functions
                ->CleanVariable($lang_cookie, "trim");
            $lang_cookie  = $this
                ->sys_functions
                ->CleanVariable($lang_cookie, "html");
            $lang_cookie  = $this
                ->sys_functions
                ->CleanVariable($lang_cookie, "sql");
            $lang_cookie  = $this
                ->sys_functions
                ->CleanVariable($lang_cookie, "intval");
          }
                ...
                if ($languageid != $MemberRows["lang"])
                  {
                    ...
                    $update       = $this
                        ->DB
                        ->sql_query("UPDATE " . $this->table["member"] . " SET lang='$languageid' WHERE id='$idMemberRows'");
                    if ($update)
                      {
                        $this
                            ->functions
        ...
        if (function_exists("inet_ntop"))
        ...
        else
          {
            ...
            case 16:
                ...
                $r = preg_replace(["/(?::?\b0+\b:?){2,}/", "/\b0+([^0])/e"], ["::", '(int)"$1"?"$1":"0$1"'], $r);
                return $r;
              }
            return false;
          }
