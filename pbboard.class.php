...
define(
    "SAFEMODE",
    (ini_get("safe_mode") == 1 or strtolower(ini_get("safe_mode")) == "on")
        ? true
        : false
);
...
if (is_array($CALL_SYSTEM)) {
    ...
    if (CLASS_NAME == "PowerBBAvatarMOD" or isset($_GET["avatar"])) {
        $files[] = $CALL_SYSTEM["AVATAR"] ? "avatar.class.php" : null;
    }
    if (
        CLASS_NAME == "PowerBBBannedMOD" or
        isset($_GET["page"]) == "register"
    ) {
    ...
    $files[] = $CALL_SYSTEM["SUPERMEMBERLOGS"]
        ? "supermemberlogs.class.php"
        : null;
    if (CLASS_NAME == "PowerBBChatMOD" or isset($_GET["page"]) == "chat") {
        $files[] = $CALL_SYSTEM["CHAT"] ? "chat.class.php" : null;
    }
    $files[] = $CALL_SYSTEM["EMAILED"]
        ? "emailed_notification.class.php"
        : null;
    if (CLASS_NAME == "PowerBBAwardMOD") {
    ...
    if (defined("IN_ADMIN")) {
        $files[] = $CALL_SYSTEM["TEMPLATESEDITS"]
            ? "templatesedits.class.php"
            : null;
    }
    $files[] = $CALL_SYSTEM["USERRATING"] ? "userrating.class.php" : null;
    if (CLASS_NAME == "PowerBBMailsendingMOD") {
        $files[] = $CALL_SYSTEM["EMAILMESSAGES"]
            ? "emailmessages.class.php"
            : null;
    }
    if (defined("IN_ADMIN")) {
        $files[] = $CALL_SYSTEM["CUSTOM_BBCODE"]
            ? "custom_bbcode.class.php"
            : null;
    }
    $files[] = $CALL_SYSTEM["CORE"] ? "core.class.php" : null;
    if (
        CLASS_NAME == "PowerBBProfileViewerMOD" or
        isset($_GET["page"]) == "profile"
    ) {
        $files[] = $CALL_SYSTEM["LOG_VISIT_PROFILE"]
            ? "profileviewer.class.php"
            : null;
    }
...
class PowerBB
{
    ...
    function __construct()
    {
        ...
        if (CLASS_NAME == "PowerBBAvatarMOD" or isset($this->_GET["avatar"])) {
            $this->avatar = $CALL_SYSTEM["AVATAR"]
                ? new PowerBBAvatar($this)
                : null;
        }
        if (CLASS_NAME == "PowerBBBannedMOD" or $page == "register") {
            $this->banned = $CALL_SYSTEM["BANNED"]
                ? new PowerBBBanned($this)
                : null;
        }
        $this->group = $CALL_SYSTEM["GROUP"] ? new PowerBBGroup($this) : null;
        $this->member = $CALL_SYSTEM["MEMBER"]
            ? new PowerBBMember($this)
            : null;
        $this->online = $CALL_SYSTEM["ONLINE"]
            ? new PowerBBOnline($this)
            : null;
        if (CLASS_NAME == "PowerBBPagesMOD") {
            $this->pages = $CALL_SYSTEM["PAGES"]
                ? new PowerBBPages($this)
                : null;
        }
        $this->pm = $CALL_SYSTEM["PM"] ? new PowerBBPM($this) : null;
        $this->reply = $CALL_SYSTEM["REPLY"] ? new PowerBBReply($this) : null;
        $this->section = $CALL_SYSTEM["SECTION"]
            ? new PowerBBSection($this)
            : null;
        $this->style = $CALL_SYSTEM["STYLE"] ? new PowerBBStyle($this) : null;
        $this->subject = $CALL_SYSTEM["SUBJECT"]
            ? new PowerBBSubject($this)
            : null;
        $this->cache = $CALL_SYSTEM["CACHE"] ? new PowerBBCache($this) : null;
        $this->misc = $CALL_SYSTEM["MISC"] ? new PowerBBMisc($this) : null;
        $this->request = $CALL_SYSTEM["REQUEST"]
            ? new PowerBBRequest($this)
            : null;
        $this->message = $CALL_SYSTEM["MESSAGE"]
            ? new PowerBBMessages($this)
            : null;
        $this->attach = $CALL_SYSTEM["ATTACH"]
            ? new PowerBBAttach($this)
            : null;
        if (CLASS_NAME == "PowerBBFixupMOD") {
            $this->fixup = $CALL_SYSTEM["FIXUP"]
                ? new PowerBBFixup($this)
                : null;
        }
        $this->extension = $CALL_SYSTEM["FILESEXTENSION"]
            ? new PowerBBFileExtension($this)
            : null;
        $this->usertitle = $CALL_SYSTEM["USERTITLE"]
            ? new PowerBBUsertitle($this)
            : null;
        $this->icon = $CALL_SYSTEM["ICONS"] ? new PowerBBIcons($this) : null;
        $this->toolbox = $CALL_SYSTEM["TOOLBOX"]
            ? new PowerBBToolBox($this)
            : null;
        $this->moderator = $CALL_SYSTEM["MODERATORS"]
            ? new PowerBBModerators($this)
            : null;
        ...
        $this->tag_subject = $CALL_SYSTEM["TAG_SUBJECT"]
            ? new PowerBBTag($this)
            : null;
        $this->warnlog = $CALL_SYSTEM["WARNLOG"]
            ? new PowerBBWarnLog($this)
            : null;
        $this->extrafield = $CALL_SYSTEM["EXTRAFIELD"]
            ? new PowerBBExtraField($this)
            : null;
        $this->lang = $CALL_SYSTEM["LANG"] ? new PowerBBLang($this) : null;
        $this->reputation = $CALL_SYSTEM["REPUTATION"]
            ? new PowerBBReputation($this)
            : null;
        $this->rating = $CALL_SYSTEM["RATING"]
            ? new PowerBBRating($this)
            : null;
        $this->supermemberlogs = $CALL_SYSTEM["SUPERMEMBERLOGS"]
            ? new PowerBBSupermemberlogs($this)
            : null;
        if (CLASS_NAME == "PowerBBChatMOD" or $page == "chat") {
            $this->chat = $CALL_SYSTEM["CHAT"] ? new PowerBBChat($this) : null;
        }
        $this->emailed = $CALL_SYSTEM["EMAILED"]
            ? new PowerBBEmailed($this)
            : null;
        if (CLASS_NAME == "PowerBBAwardMOD") {
            $this->award = $CALL_SYSTEM["AWARD"]
                ? new PowerBBAward($this)
                : null;
        }
        $this->adsense = $CALL_SYSTEM["ADSENSE"]
            ? new PowerBBAdsense($this)
            : null;
        $this->friends = $CALL_SYSTEM["FRIENDS"]
            ? new PowerBBFriends($this)
            : null;
        if (defined("IN_ADMIN")) {
            $this->addons = $CALL_SYSTEM["ADDONS"]
                ? new PowerBBAddons($this)
                : null;
        }
        if (defined("IN_ADMIN")) {
            $this->hooks = $CALL_SYSTEM["HOOKS"]
                ? new PowerBBHooks($this)
                : null;
        }
        if (defined("IN_ADMIN")) {
            $this->templates_edits = $CALL_SYSTEM["TEMPLATESEDITS"]
                ? new PowerBBTemplatesEdits($this)
                : null;
        }
        $this->userrating = $CALL_SYSTEM["USERRATING"]
            ? new PowerBBUserRating($this)
            : null;
        if (CLASS_NAME == "PowerBBMailsendingMOD") {
            $this->emailmessages = $CALL_SYSTEM["EMAILMESSAGES"]
                ? new PowerBBEmailMessages($this)
                : null;
        }
        if (CLASS_NAME == "PowerBBCustom_bbcodeMOD") {
            $this->custom_bbcode = $CALL_SYSTEM["CUSTOM_BBCODE"]
                ? new PowerBBCustom_bbcode($this)
                : null;
        }
        if (CLASS_NAME == "PowerBBProfileViewerMOD" or $page == "profile") {
            $this->log_profile_visit = $CALL_SYSTEM["LOG_VISIT_PROFILE"]
                ? new PowerBBProfileViewer($this)
                : null;
        }
    ...
    function _GetInfoRows()
    {
        ...
        while ($x <= $y) {
            @$this->_CONF["info_row"][$rows[$x]["var_name"]] =
                $rows[$x]["value"];
    ...
    function _GetLangRows()
    {
        if (isset($this->_COOKIE[$this->_CONF["lang_cookie"]])) {
            $lang_cookie = $this->sys_functions->CleanSymbols(
                $this->_COOKIE[$this->_CONF["lang_cookie"]]
            );
            $lang_cookie = $this->sys_functions->CleanVariable(
                $lang_cookie,
                "trim"
            );
            $lang_cookie = $this->sys_functions->CleanVariable(
        ...
        if (empty($username)) {
            ...
            if ($MemberRows == true) {
                if ($languageid != $MemberRows["lang"]) {
                    ...
                    $update = $this->DB->sql_query(
                        "UPDATE " .
                            $this->table["member"] .
                            " SET lang='$languageid' WHERE id='$idMemberRows'"
                    );
                    if ($update) {
        ...
        while ($x <= $y) {
            ...
            if (isset($rows[$x]["varname"]) == "post_text_max") {
                $rows[$x]["text"] = str_replace(
                    "30000",
                    $this->_CONF["info_row"]["post_text_max"],
                    $rows[$x]["text"]
                );
            } elseif (isset($rows[$x]["varname"]) == "floodctrl") {
                $rows[$x]["text"] = str_replace(
                    "30",
                    $this->_CONF["info_row"]["floodctrl"],
                    $rows[$x]["text"]
                );
            } elseif (isset($rows[$x]["varname"]) == "Reply_Editing_time_out") {
                $rows[$x]["text"] = str_replace(
                    "1440",
                    $this->_CONF["info_row"]["time_out"],
                    $rows[$x]["text"]
                );
                ...
                isset($rows[$x]["varname"]) == "characters_keyword_search"
            ) {
                $rows[$x]["text"] = str_replace(
                    "3",
                    $this->_CONF["info_row"]["characters_keyword_search"],
                    $rows[$x]["text"]
                );
            } elseif (isset($rows[$x]["varname"]) == "flood_search1") {
                $rows[$x]["text"] = str_replace(
                    "40",
                    $this->_CONF["info_row"]["flood_search"],
                    $rows[$x]["text"]
                );
            } elseif (isset($rows[$x]["varname"]) == "max_avatar") {
                $rows[$x]["text"] = str_ireplace(
                    "150 في",
                    $this->_CONF["info_row"]["max_avatar_width"] . " في",
                    $rows[$x]["text"]
                );
                $rows[$x]["text"] = str_ireplace(
                    "150 بيكسل",
                    $this->_CONF["info_row"]["max_avatar_height"] . " بيكسل",
                    $rows[$x]["text"]
                );
                $rows[$x]["text"] = str_ireplace(
                    "150 in",
                    $this->_CONF["info_row"]["max_avatar_width"] . " in",
                    $rows[$x]["text"]
                );
                $rows[$x]["text"] = str_ireplace(
                    "150 Pixel",
                    $this->_CONF["info_row"]["max_avatar_height"] . " Pixel",
                    $rows[$x]["text"]
                );
            } elseif (isset($rows[$x]["varname"]) == "post_text_max_subjects") {
                $rows[$x]["text"] = str_replace(
                    "60",
                    $this->_CONF["info_row"]["post_title_max"],
                    $rows[$x]["text"]
                );
            } elseif (isset($rows[$x]["varname"]) == "post_text_min_subjects") {
                $rows[$x]["text"] = str_replace(
                    "(4)",
                    $this->_CONF["info_row"]["post_title_min"],
                    $rows[$x]["text"]
                );
                $rows[$x]["text"] = str_replace(
                    "60",
                ...
                $rows[$x]["text"] = str_replace(
                    "5",
                    $this->_CONF["info_row"]["post_text_min"],
                    $rows[$x]["text"]
                );
            } elseif (
                isset($rows[$x]["varname"]) == "Character_name_a_few_user"
            ) {
                $rows[$x]["text"] = str_replace(
                    "3",
                    $this->_CONF["info_row"]["reg_less_num"],
                    $rows[$x]["text"]
                );
            } elseif (
                isset($rows[$x]["varname"]) == "characters_Username_many"
            ) {
                $rows[$x]["text"] = str_replace(
                    "25",
                    $this->_CONF["info_row"]["reg_max_num"],
                    $rows[$x]["text"]
                );
            } elseif (isset($rows[$x]["varname"]) == "Character_pass_few") {
                $rows[$x]["text"] = str_replace(
                    "5",
                    $this->_CONF["info_row"]["reg_pass_min_num"],
                    $rows[$x]["text"]
                );
            } elseif (isset($rows[$x]["varname"]) == "Character_pass_many") {
                $rows[$x]["text"] = str_replace(
                    "25",
                    $this->_CONF["info_row"]["reg_pass_max_num"],
                    $rows[$x]["text"]
                );
            } elseif (
                isset($rows[$x]["varname"]) == "send_a_private_message_to"
            ) {
                $rows[$x]["text"] = str_replace(
                    $rows[$x]["text"],
                    $rows[$x]["text"] . ": ",
                ...
                isset($rows[$x]["varname"]) == "send_a_message_to_the_mailing"
            ) {
                $rows[$x]["text"] = str_replace(
                    $rows[$x]["text"],
                    $rows[$x]["text"] . ": ",
    ...
    function get_ip()
    {
        ...
        if (isset($this->_SERVER["HTTP_X_FORWARDED_FOR"])) {
            $addresses = explode(
                ",",
                strtolower($this->_SERVER["HTTP_X_FORWARDED_FOR"])
            );
        } elseif (isset($this->_SERVER["HTTP_X_REAL_IP"])) {
            $addresses = explode(
                ",",
                strtolower($this->_SERVER["HTTP_X_REAL_IP"])
            );
        }
        ...
        if (is_array($addresses)) {
            foreach ($addresses as $val) {
                ...
                if (
                    $this->my_inet_ntop($this->my_inet_pton($val)) == $val &&
                    !preg_match(
                        "#^(10\.|172\.(1[6-9]|2[0-9]|3[0-1])\.|192\.168\.|fe80:|fe[c-f][0-f]:|f[c-d][0-f]{2}:)#",
                        $val
                    )
                ) {
                    $ip = $val;
